...
export function Avatar({ src, name, className }) {
  ...
  if (src && !failed) {
    return (
      <img
        ...
        className={cn(
          "h-10 w-10 shrink-0 rounded-full border border-border bg-card-hover object-cover",
          className,
        )}
      />
    );
  ...
  return (
    <span
      aria-hidden
      className={cn(
        "grid h-10 w-10 shrink-0 place-items-center rounded-full border border-border bg-card-hover text-sm font-bold",
        className,
      )}
    >
      {initial}
      ...
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex items-center gap-3">
          ...
          {Array.from({ length: Math.max(cols - 2, 0) }).map((_, j) => (
            <div
              key={j}
              className="hidden h-3.5 w-20 animate-pulse rounded bg-card-hover sm:block"
            />
          ))}
        </div>
      ))}
...
export function EmptyState({ title = "ยังไม่มีข้อมูล", hint, action }) {
  return (
    <div className="grid place-items-center gap-2 px-6 py-12 text-center">
      <div
        className="grid h-12 w-12 place-items-center rounded-full border border-dashed border-border text-xl text-muted-foreground"
        aria-hidden
      >
        ○
      </div>
